...
class TestLiczbyNaturalne(unittest.TestCase):
  ...
  def test_07_pow(self):
    """
    Test sprawdza czy poprawie zaimplementowane jest potęgowanie liczb naturalnych
    """
    l1 = LiczbaNaturalna("2")
    self.assertEqual(str(l1**2),"4","Potęgowanie liczby jedno cyfowych nie jest poprawnie zaimplementowane")

    with self.assertRaises(ValueError,msg="Brak sprawdzenia czy wykładnik nie jest liczbą ujemną"):
      l1 = LiczbaNaturalna("2")
      l3 = l1 ** (-2)

    l1 = LiczbaNaturalna("2")
    self.assertEqual(str(l1**0),"1","Potęgowanie liczby do potęgi 0 nie jest poprawnie relizowane")
