...
class LiczbaNaturalna():
  ...
  def __lshift__(self,shift : int) -> "LiczbaNaturalna":
    cyfry_wyniku: list[int] = list(chain(repeat(0, shift), self.cyfry))
    return LiczbaNaturalna(cyfry_wyniku)

  def __ilshift__(self,shift : int) -> "LiczbaNaturalna":
    self.cyfry = list(chain(repeat(0, shift), self.cyfry))
    return self
  
  def __mul__(self, druga_liczba : "LiczbaNaturalna") -> "LiczbaNaturalna":
    """
    Operator mnożenia
    """
    wynik = LiczbaNaturalna("0")
    for i, cyfra_drugiej_liczby in enumerate(druga_liczba.cyfry):
      cyfry_wyniku: list[int] = []
      przeniesienie : int = 0    
      for cyfra_pierwszej_liczby in self.cyfry:
        iloczyn : int
        iloczyn = cyfra_pierwszej_liczby * cyfra_drugiej_liczby + przeniesienie
        cyfra_wyniku = iloczyn % 10
        przeniesienie = iloczyn // 10
        cyfry_wyniku.append(cyfra_wyniku)  
      if przeniesienie: # Jeśli po dodawaniu nadal jest przeniesienie dodaj cyfrę do wyniku
        cyfry_wyniku.append(przeniesienie)
        
      liczba = LiczbaNaturalna(cyfry_wyniku)
      liczba = liczba << i
      wynik = wynik + liczba
      
    return wynik  

  def __pow__(self, wykladnik : int) -> "LiczbaNaturalna":
    """
    Operator potęgowania z int
    """
    if wykladnik < 0:
      raise ValueError(f"Wykładnik dla liczb naturalnych nie może być liczbą ujemną: podano wykładnik={wykladnik}")
    if wykladnik == 0:
      return LiczbaNaturalna("1")
